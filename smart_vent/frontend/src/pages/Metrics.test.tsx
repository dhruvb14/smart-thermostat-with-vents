...
describe("Metrics Page", () => {
  beforeEach(() => {
    ...
    (api.getMetricsThermostatSummary as any).mockResolvedValue({ ...mockSummary, thermostat_entity_id: "climate.test" });
    (api.getOutsideTempEntity as any).mockResolvedValue({ entity_id: "sensor.outside_temp", current_value: 42 });
    (api.getHAEntities as any).mockResolvedValue([{ entity_id: "sensor.outside_temp", friendly_name: "Outside Temp" }]);
    (api.setOutsideTempEntity as any).mockResolvedValue({ entity_id: "sensor.outside_temp", current_value: 42 });
    ...
    (api.getMetricsOvershootHistogram as any).mockResolvedValue({ labels: [], counts: [], total_room_cycles: 0 });
  ...
  it("changes thermostat filter", async () => {
    ...
    await waitFor(() => {
        expect(api.getMetricsThermostatSummary).toHaveBeenCalledWith("climate.test", expect.anything());
    });
  });
