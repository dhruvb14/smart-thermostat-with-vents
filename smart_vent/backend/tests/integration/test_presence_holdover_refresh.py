...
@pytest.mark.asyncio
async def test_sensor_off_does_not_arm_or_extend_holdover(client, fake_ha, tick) -> None:
    """#433 (always-on off-gate): a presence sensor reading "off" must neither
    arm a holdover nor extend a near-expiry one — the pre-#287 expiry path.
    A regression here keeps every room presence-active forever."""
    thermostat = "climate.test_thermostat"
    presence = "binary_sensor.room_presence"
    ...
    fake_ha.seed_state(presence, "off", {})
    ...
    # No holdover exists and the "off" sensor must not create one.
    await tick()
    assert await db.get_holdover_state(conn, room_id) is None, (
        "an 'off' sensor must not arm a holdover"
    )

    # A near-expiry holdover must NOT be extended by an "off" sensor...
    from backend.models import PresenceHoldoverState

    now = datetime.now(UTC)
    near_expiry = now + timedelta(seconds=30)
    await db.upsert_holdover_state(
        conn,
        PresenceHoldoverState(room_id=room_id, last_detected_at=now, expires_at=near_expiry),
    )
    await tick()
    holdover = await db.get_holdover_state(conn, room_id)
    assert holdover is not None and holdover.expires_at == near_expiry, (
        "an 'off' sensor must not extend the holdover"
    )

    # ...and once expired, the room goes idle (the row is expired away).
    await db.upsert_holdover_state(
        conn,
        PresenceHoldoverState(
            room_id=room_id,
    ...
    await tick()
    assert await db.get_holdover_state(conn, room_id) is None, (
        "an expired holdover must be swept, returning the room to idle"
    )
