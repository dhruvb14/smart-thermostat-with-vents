...
- Path: `${DATA_DIR}/app.db`. `DATA_DIR` defaults to `/data` in the backend
  (`smart_vent/backend/main.py:52-53`) and is exported as `/config` by the
  add-on's `run.sh` (as of 2026-09, v0.35.0). Local dev: wherever you pointed
  `DATA_DIR`.
...
### Schema map (tables you'll actually query — full DDL in `smart_vent/backend/db.py` `SCHEMA` (lines 43–322) + `MIGRATIONS` (from line 742))
...
| `presence_suppression` | one active "clear presence" suppression per room (#439) — while a row exists, presence demand is ignored until every presence sensor for the room reads off | PK `room_id`, `cleared_at` |
| `eco_suspensions` | one active Eco Suspend per thermostat (#500/#501) — Eco Mode disabled until `resume_at` | PK `thermostat_entity_id`, `resume_at` |
| `mcp_tokens` | one issued MCP bearer token (#373 auth) | `id`, `label`, `token_hash`, `scope`, `created_at`, `last_used_at` |
...
### `ended_reason` taxonomy (verified in `cycle_engine.py`, as of v0.35.0)
...
(the last three are the #237/#254 overflow-conditioning actions), and
`reopened_drift` — a served room (`vent_closed_at` already set) whose temp
drifts back past its deadband mid-cycle gets its vent reopened rather than
staying closed and ignored (#503).
...
the engine as of v0.35.0** — don't expect those rows in fresh data.
...
Categories actually emitted (verified by grepping every `.log(`/`emit(` call site, v0.35.0):
...
| `auth` | routes (`emit()` calls in the login/OIDC/MCP-token handlers, #373) | direct-port login success/failure, OIDC sign-in, MCP token issue/revoke |
...
The UI filter list at `Logs.tsx:850` — `CATEGORIES = ["all","system","api","engine","presence","ha","dev","reconcile"]`
— is **missing `auth`**: auth events still land in `event_log` and the Live
Feed under "all", but there's no dedicated filter chip for them (see
`suspected_bugs`). Levels: `info`/`warning`/`error`.
...
| Degree-minutes | `_degree_minutes_timeseries`: trapezoid-free left-sum of \|setpoint − thermostat_temp\| over inter-sample gaps, **using thermostat-level samples (`room_id IS NULL`) only** | "How much conditioning was demanded." CAVEAT: as of v0.35.0 the engine's only `insert_cycle_temp_sample` call site (`cycle_engine.py:1368`) writes per-room rows — verify NULL-room rows exist (`SELECT COUNT(*) FROM cycle_temp_samples WHERE room_id IS NULL`) before trusting an empty chart. |
...
| Eco Mode impact (tiles + charts, #442) | `compute_eco_impact` (`db.py:2946`, Issue #404/#442): splits cycles/runtime by whether Eco Mode relaxed the target, reports avg applied drift (°F) and a per-day series, fed to `MetricsCharts.tsx`'s `EcoImpactSection` | Deliberately does not claim a kWh number — Plenum can't read energy usage, only infer savings from runtime reduction. |
...
| `POST /api/ha/states` `{"entity_ids":[…]}` | HA state-cache snapshot; **°C sensor values are normalized to °F** in `numeric` (routes.py `ha_states`, ~2069-2098) — `numeric` is always °F regardless of display unit |
| `GET /api/sensor-health` | rooms with stale temp sensors (#211): `age_seconds` per sensor, `reason` `stale` or `not_in_cache`; threshold = `sensor_stale_after_min` setting, default 30 min (`SENSOR_STALE_AFTER_MIN`, cycle_engine.py:56). Healthy rooms omitted. |
...
Types emitted (verified broadcast call sites, v0.35.0):
...
- `system_enabled_changed` / `dev_mode_changed` / `mcp_enabled_changed` /
  `mqtt_enabled_changed` (#519) / `theme_changed` (#458) — all broadcast from
  `scheduler.py`.
...
DB built from the real `db.py` DDL (`SCHEMA` + `MIGRATIONS`) with 7 days of
synthetic cycles.
...
Facts re-verified 2026-09-01 against v0.35.0 source (originally verified
2026-07-04 against v0.22.1; schema/line-number facts refreshed, script
inventory and behavior unchanged). Re-verification commands (repo root):

- Schema / DDL: `grep -n "CREATE TABLE" smart_vent/backend/db.py` (`SCHEMA` spans lines 43–322 as of 2026-09) and `grep -n "^MIGRATIONS" smart_vent/backend/db.py` (renamed from `_MIGRATIONS`; the tuple starts at line 742). If tables/columns change, rebuild the scripts' expectations.
- Event categories: `grep -rn -A2 '\.log($' smart_vent/backend --include="*.py" | grep -v tests | grep -oE '"\w+"'` for the `EventLogger.log()` call sites, plus `grep -n 'await emit(' smart_vent/backend/api/routes.py` for the routes-layer wrapper — currently engine, system, api, auth, ha, presence, dev, reconcile; UI filter list at `smart_vent/frontend/src/pages/Logs.tsx:850` (still missing `auth` as of 2026-09 — see suspected-bug note above).
...
- Vent-event actions: `grep -n 'insert_cycle_vent_event' smart_vent/backend/engine/cycle_engine.py` then read each call's action string. `closed_at_end` still has no emit site → if one appears, update §1. `reopened_drift` (#503) was added since the 2026-07-04 pass.
- Degree-minutes NULL-sample caveat: `grep -rn insert_cycle_temp_sample smart_vent/backend --include='*.py' | grep -v tests` — still one call site (per-room) as of v0.35.0; recheck whether a thermostat-level (`room_id=None`) writer was added.
- WS event types: `grep -rn "_broadcast(\"" smart_vent/backend/scheduler.py smart_vent/backend/engine/cycle_engine.py` — `mqtt_enabled_changed` (#519) and `theme_changed` (#458) were added since the 2026-07-04 pass.
- Eco Mode impact chart: `grep -n "^async def compute_eco_impact" smart_vent/backend/db.py` (#442) — added since the 2026-07-04 pass, was missing from the Metrics chart table.
- Metrics endpoints & chart list: `grep -n '@routes.get("/api/metrics' smart_vent/backend/api/routes.py`; `docs/metrics.md`.
- Constants: event-log cap `grep -n _EVENT_LOG_MAX smart_vent/backend/db.py` (5000); staleness default `grep -n "SENSOR_STALE_AFTER_MIN" smart_vent/backend/engine/cycle_engine.py` (30.0, now at line 56); ports `grep -n 8099 smart_vent/config.yaml`.
- Scripts smoke test: build a throwaway DB by extracting `SCHEMA`/`MIGRATIONS` from `db.py` via `ast.literal_eval`, seed a few cycles, and run each script against it (this is how they were validated; they were **not** run against a production DB).

Known limits (as of 2026-07, re-checked 2026-09 — still true): scripts' `--days` filters compare against UTC
