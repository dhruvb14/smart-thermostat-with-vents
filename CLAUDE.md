...
- **On refetch**: re-derive only when the derived **content** changes, and do it **during render** — never reset form state from the prop in a `useEffect`. A refetch (`load()` after a delete, a registration, an Eco suspend) hands every card a new object with identical values, so an identity-keyed reset discards edits the user has not saved; and React schedules passive effects as a separate host task, so a reset queued in an effect can land *after* a keystroke and replace it. See pitfall #9 and `ThermostatCard` in `Thermostats.tsx` (#597).
...
```
Coverage threshold: **99.9%** (`pyproject.toml` `fail_under = 99.9`). Subset runs
...
**Docs-only path limiting.** All five jobs above are gated behind a `changes`
job (job-level `if:`, never a workflow-level `paths` filter — a filtered-out
required check hangs the PR forever; a job skipped by `if:` reports "skipped",
which satisfies branch protection). A PR whose diff touches **only** root-level
markdown (`*.md`), `docs/`, `.claude/`, `.jules/`, `.vscode/`, `screenshots/`,
or `e2e/screenshots/` (goldens) skips lint/test/scan entirely. `.github/` is
deliberately **code** — a workflow edit must run the full suite so CI validates
itself. Nested markdown outside those dirs (e.g. `smart_vent/CHANGELOG.md`) is
also code. The identical ignore set gates the container pipeline (container-ci's
`changes` job, including `Build (PR validation)`), the beta-pointer bump
(`beta.yml`), `docker.yml` (push-to-main `paths-ignore`), and the local
...
9. **Deriving form state from a prop or context** — a `useState` seeded from a prop plus a `useEffect` that re-derives it is a data-loss bug waiting to happen, and it caused #597. Two rules. **Key the re-derive on the derived CONTENT, never on object identity**: a refetch (`load()` after a delete, a registration, an Eco suspend) hands every card a brand-new object with identical values, and re-deriving on identity silently discards whatever the user had typed into the *other* cards — #293's bug one path over. **Do the re-derive during render, not in a `useEffect`**: React commits the DOM and schedules passive effects as a separate host task, so a reset queued in an effect can flush *after* an input's `onChange` has queued its update, and `setForm(<plain object>)` then replaces the edit no matter how it was written — a functional updater does not save you. Use React's "adjusting state when a prop changes" with a `lastDerived` **state** cell (not a ref — no ref writes during render) so StrictMode's double render converges; `main.tsx` wraps the app in `<StrictMode>`, where a guard that merely skips the effect's first run still clobbers. `ThermostatCard` in `Thermostats.tsx` is the worked example. The related trap — a **different mechanism**, so don't fix it with the content-keyed re-derive above: filling an editable input from a mount fetch with no loading gate lets a late resolve overwrite typing, and a failed fetch quietly writes a hardcoded default over the operator's configured value. Gate on `loading` like `SensorStalenessCard` in `Thermostats.tsx` (#600) and `RetentionSettings` in `Logs.tsx` (#605): seed the state with the backend's own default (never `null`) and **name that default as a module constant**, so the seed and the warning copy below cannot drift; the failure path is the only place the seed is ever visible, so pin both the seeded value and the whole warning sentence in the failure-path test or a divergence between them ships unnoticed. Keep a separate `loading` cell starting `true` and render no input at all until it clears; have `.then` set the value **and** clear the gate; have `.catch` clear the gate **and** set a dedicated `loadFailed` cell that renders a warning naming the fabricated value(s) out loud. `loadFailed` must be its own cell, never the component's `error`/`status` cell: `save()` clears that one, and a warning that the shown number is a fabricated default must not be erased by the very Save it exists to warn about. Only a **successful** write clears `loadFailed` — that is the moment the field starts holding what the server holds; a rejected Save leaves the numbers fabricated. Both components are deliberately warn-only (Save stays live under `loadFailed`), both disable their input(s) while the write is in flight (`disabled={saving}`) because `save()` adopts the server's echo and that write would land on anything typed meanwhile, and both cancel the mount fetch from the effect's cleanup with a `cancelled` flag, because `<StrictMode>` runs setup → cleanup → setup and the loser's response would otherwise land in a control the winner already made editable. Write the warning as a **single template literal** — JSX interpolation splits a message across text nodes and `getByText` then cannot match it. A `value === null` early-return in the save handler is the smell that this gate is missing — it makes Save a silent no-op instead.
