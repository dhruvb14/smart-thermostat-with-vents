...
- **Pin `scrollbar-gutter: stable` (fixtures.ts auto-fixture) or tall pages flap.** A fullPage capture renders the page on one surface with no vertical scrollbar, so content re-wraps ~15px wider than the viewport render; text on a wrap boundary makes the page height bi-stable and Playwright's "two consecutive stable screenshots" check times out ~50% of runs (seen as 6799↔6801px on the Thermostats settings panel). The shared fixture reserves the gutter so both layouts are identical — don't remove it.
