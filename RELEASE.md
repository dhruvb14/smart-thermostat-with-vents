...
2. Confirm the Trivy image scan (a step of that same `Build (PR validation)` job) shows no CRITICAL vulnerabilities. On a release PR the scan runs there rather than in the separate `Image vulnerability scan` job (#596) precisely so a CRITICAL fails the required check that gates the publish; both use the shared `.github/actions/scan-image` action, so the severity gate is identical
...
| Open PR → main (non-release) | `container-ci.yml` → `Image vulnerability scan` | Trivy-scans the `ci-<sha>` image the build just produced (fork PRs: the `docker load`ed artifact), so base-image CVEs surface on the PR that could fix them instead of piling up until the next release PR (#596). CRITICAL fails; HIGH is reported to the job summary and the Security tab without blocking. Skipped on release PRs — `Build (PR validation)` scans the published image there. |
